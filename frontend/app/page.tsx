...
function OutreachView({ client, profile, jobs, applications, notify, refresh, setView }: ViewProps) {
  ...
  return <div className="aa-stack"><SectionHeader eyebrow="Review-gated outreach" title="Email outreach" text="Generate a research brief, import the workbook, choose roles that already have email leads, then review before sending." action={<Button onClick={() => setView("applications")} className="aa-button-secondary">Review & send</Button>} /><div className="aa-prerequisites"><span className="aa-prereq aa-prereq-ready">✓ Résumé parsed</span><span className="aa-prereq">○ Groq key · Profile or Connections</span><span className="aa-prereq">○ Gmail connected</span></div><section className="aa-panel"><SectionHeader eyebrow="Step 1 · External AI research" title="Generate the 100-lead research brief" text="The output targets 100 distinct email leads across at least 25 companies, with no more than four contacts per company. Contacts are not necessarily HR: recruiters, founders, engineering leaders, team members, or company inboxes are valid; source-verified status is preferred but not required." /><form className="aa-form" onSubmit={generatePrompt}><div className="aa-field-grid"><Field label="Target role (optional)" value={targetRole} onChange={setTargetRole} placeholder="AI Engineer or Backend Engineer" /><Field label="Location" value={location} onChange={setLocation} placeholder="New Delhi, India" /></div><label className="aa-checkbox"><input type="checkbox" checked={remoteOnly} onChange={(event) => setRemoteOnly(event.target.checked)} /><span>Remote roles only</span></label><Button type="submit" busy={busy === "prompt"} className="aa-button-primary">Generate 100-lead prompt</Button></form>{prompt?.prompt && <div className="aa-prompt-output"><div className="aa-section-header"><div><strong>Copy into Claude, ChatGPT, or Gemini</strong><small>{stringValue(prompt.summary, "100-lead research brief")}</small></div><Button onClick={() => void copyPrompt()} className="aa-button-secondary">Copy prompt</Button></div><textarea value={prompt.prompt} readOnly rows={18} aria-label="External AI research prompt" /></div>}</section><section className="aa-panel"><SectionHeader eyebrow="Step 2 · Import" title="Upload the completed workbook" text="Choose the workbook created by Claude, ChatGPT, or Gemini. We check the columns before importing." /><form className="aa-import-card" onSubmit={importWorkbook}><label className="aa-dropzone aa-dropzone-small"><input type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => setFile(event.target.files?.[0] || null)} /><span className="aa-upload-icon">↑</span><strong>{file ? file.name : "Choose completed CSV or XLSX"}</strong><small>{file ? bytesLabel(file.size) : "The prompt creates the workbook; upload it here when ready."}</small></label><Button type="submit" busy={busy === "import"} className="aa-button-primary">Import into Jobs</Button></form></section><section className="aa-panel"><div className="aa-step-heading"><span>3</span><div><p className="aa-eyebrow">Choose deliberately</p><h3>Select roles with email leads</h3><p>Only roles with an email address are shown. Maximum 30 roles per batch.</p></div><span className="aa-count-badge">{selected.length} / 30</span></div><div className="aa-outreach-list">{candidates.map((job) => { const index = selected.indexOf(job.id); const app = appFor(job.id); return <label className={`aa-outreach-row ${index >= 0 ? "aa-row-active" : ""}`} key={job.id}><input type="checkbox" checked={index >= 0} onChange={() => toggle(job.id)} /><span className="aa-order">{index >= 0 ? String(index + 1).padStart(2, "0") : "·"}</span><span className="aa-list-copy"><strong>{stringValue(job.title, "Untitled role")}</strong><span>{stringValue(job.company)}{job.location ? ` · ${job.location}` : ""}</span></span>{job.fit?.score ? <span className="aa-fit-pill">{job.fit.score}% fit</span> : <Status value={app?.status || job.status} />}</label>; })}{!candidates.length && <Empty title="No roles with email leads" text="Import a workbook with an email column first. Roles without an address stay out of this list." />}</div></section><section className="aa-panel"><div className="aa-step-heading"><span>4</span><div><p className="aa-eyebrow">Review selected leads</p><h3>Email leads</h3><p>Choose one address per role. Nothing is sent until you approve the draft.</p></div></div><div className="aa-field-grid aa-contact-limits"><Field label="Leads per job" value={contactLimit} onChange={(value) => setContactLimit(Number(value))} type="number" min={1} max={50} /><Field label="Pages per site" value={contactPages} onChange={(value) => setContactPages(Number(value))} type="number" min={1} max={12} /><Field label="Time limit (seconds)" value={contactTimeout} onChange={(value) => setContactTimeout(Number(value))} type="number" min={15} max={120} /></div><div className="aa-button-row"><Button onClick={() => void findContacts()} busy={busy === "contacts"} disabled={!selected.length} className="aa-button-primary">Find contacts</Button><Button onClick={() => void createDrafts()} busy={busy === "drafts"} disabled={!selected.length} className="aa-button-secondary">Create Groq drafts</Button><Button onClick={() => void sendApproved()} busy={busy === "send"} disabled={!selected.length} className="aa-button-secondary">Queue approved emails</Button></div><div className="aa-contact-list">{selected.map((id) => { const job = jobs.find((item) => item.id === id); if (!job) return null; const found = contacts[id] || []; return <div className="aa-contact-row" key={id}><div className="aa-list-copy"><strong>{stringValue(job.company)}</strong><span>{found.length ? `${found.length} email lead${found.length === 1 ? "" : "s"}` : "Not searched yet"}</span></div>{found.length ? <select value={chosen[id] || ""} onChange={(event) => setChosen((current) => ({ ...current, [id]: event.target.value }))}><option value="">Choose an email</option>{found.map((contact) => <option value={contact.email} key={`${contact.email}-${contact.name}`}>{contact.email}{contact.name ? ` · ${contact.name}` : ""}</option>)}</select> : <span className="aa-muted">{job.contact_email || "No email lead"}</span>}</div>; })}{!selected.length && <Empty title="Select a role with an email" text="Only roles with an available email address can be selected." />}</div></section></div>;
}
