...
type View = "overview" | "profile" | "discovery" | "form_pilot" | "outreach" | "jobs" | "connections" | "activity" | "settings";
...
function JobsView({ client, jobs, setJobs, notify, refresh, setView }: ViewProps) {
  ...
  const draft = async (job: Job) => { if (!job.contact_email) { notify("This role has no email lead. Open Email leads and choose a role with an address.", "error"); return; } setBusy(`draft-${job.id}`); try { await apiRequest(client, `/jobs/${encodeURIComponent(job.id)}/draft`, { method: "POST", body: { recipient: job.contact_email } }); await refresh(); notify("Draft created. Open Email leads to review it.", "success"); setView("outreach"); } catch (error) { notify(errorMessage(error, "The draft could not be generated. Add Groq in Connections."), "error"); } finally { setBusy(""); } };
...
function ApplicationsView({ client, applications, jobs, setApplications, notify, refresh, embedded = false, focusApplicationId }: Pick<ViewProps, "client" | "applications" | "jobs" | "setApplications" | "notify" | "refresh"> & { embedded?: boolean; focusApplicationId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const application = applications.find((item) => item.id === selectedId) || null;
    const fallbackRecipient = application && !application.recipient ? selectedJob?.contact_email : null;
    setDraft(application && fallbackRecipient ? { ...application, recipient: fallbackRecipient } : application);
  }, [applications, selectedId, selectedJob?.contact_email]);
  useEffect(() => {
    if (focusApplicationId) setSelectedId(focusApplicationId);
  }, [focusApplicationId]);
  const recipientFor = (application: Application) => application.recipient || jobs.find((job) => job.id === application.job_id)?.contact_email || "";
  const rows = applications.filter((item) => (!embedded || (item.channel === "email" && Boolean(recipientFor(item)))) && (filter === "all" || item.status === filter));
  ...
  const approve = async () => {
    ...
    if (!draft.recipient?.trim()) {
      notify("Add a recipient email before approving this draft.", "error");
      return;
    }
    setBusy("approve");
    try {
      let approvalTarget = draft;
      const storedApplication = applications.find((item) => item.id === draft.id);
      if (draft.recipient !== storedApplication?.recipient) {
        const saved = await apiRequest<{ data: Application }>(client, `/applications/${encodeURIComponent(draft.id)}`, { method: "PATCH", body: { recipient: draft.recipient, subject: draft.subject || null, body: draft.body || null } });
        approvalTarget = unwrapData(saved) || draft;
        setApplications((current) => current.map((item) => item.id === approvalTarget.id ? approvalTarget : item));
        setDraft(approvalTarget);
      }
      const result = await apiRequest<{ data: Application }>(client, `/applications/${encodeURIComponent(approvalTarget.id)}/approve`, { method: "POST", body: { expected_revision: numberValue(approvalTarget.revision, 1) } });
      const updated = unwrapData(result);
      if (updated) { setApplications((current) => current.map((item) => item.id === updated.id ? updated : item)); setDraft(updated); }
      notify("Message approved. Sending remains a separate explicit action.", "success");
    } catch (error) { notify(errorMessage(error, "The draft changed; refresh and review it again."), "error"); } finally { setBusy(""); }
  };
...
function OutreachView({ client, profile, jobs, applications, credentials, setApplications, notify, refresh, setView }: ViewProps) {
  ...
  const openDraft = (applicationId: string) => {
    setFocusApplicationId(applicationId);
    window.setTimeout(() => document.getElementById("email-review")?.scrollIntoView({ behavior: "smooth", block: "start" }), 30);
  };
  ...
  const findContacts = async () => {
    if (!selected.length) { notify("Select at least one role first.", "error"); return; }
    setBusy("contacts");
    try {
      const result = await apiRequest<{ data?: { contacts?: Record<string, Contact[]>; automation_job?: AutomationJob } }>(client, "/contacts/discover", {
        method: "POST",
        body: {
          job_ids: selected,
          max_contacts_per_job: contactLimit,
          max_pages_per_job: contactPages,
          timeout_seconds: contactTimeout,
          idempotency_key: idempotencyKey("public-contacts"),
        },
      });
      const queued = unwrapData(result);
      setContacts((current) => ({ ...current, ...(queued?.contacts || {}) }));
      const automationJobId = queued?.automation_job?.id;
      setBusy("");
      notify("Contact search started. We will show public leads as the worker finds them.", "info");

      // Poll the small status endpoint, never the crawler itself. This keeps the
        ...
        void (async () => {
          for (let attempt = 0; attempt < 40; attempt += 1) {
            ...
            const rows = await Promise.allSettled(selected.map(async (id) => {
              const response = await apiRequest<{ data?: { contacts?: Contact[] } }>(client, `/jobs/${encodeURIComponent(id)}/contacts/public`);
              return [id, unwrapData(response)?.contacts || []] as const;
            }));
            const found = rows.reduce((total, row) => total + (row.status === "fulfilled" ? row.value[1].length : 0), 0);
            setContacts((current) => {
              const next = { ...current };
              rows.forEach((row) => { if (row.status === "fulfilled") next[row.value[0]] = row.value[1]; });
              return next;
            });
            if (["succeeded", "failed", "cancelled", "needs_attention"].includes(status)) {
              notify(found ? `${found} public contact lead${found === 1 ? "" : "s"} ready for review.` : "Search finished, but no public email was found on the allowed pages.", found ? "success" : "info");
              break;
            }
          }
        })();
      ...
      setBusy("");
      notify(errorMessage(error, "The public contact search could not be queued."), "error");
    }
  };
  ...
  const createDrafts = async () => {
    ...
    if (!selectedJobs.length || missing.length) {
      notify("Select an email lead for every chosen role before drafting.", "error");
      return;
    }
    setBusy("drafts");
    let count = 0;
    ...
    for (const job of selectedJobs) {
      ...
      try {
        await apiRequest(client, `/jobs/${encodeURIComponent(job.id)}`, { method: "PATCH", body: { contact_email: recipient } });
        const result = await apiRequest<{ data?: Application }>(client, `/jobs/${encodeURIComponent(job.id)}/draft`, { method: "POST", body: { recipient } });
        const created = unwrapData(result);
  ...
  return <div className="aa-stack"><SectionHeader eyebrow="Review-gated outreach" title="Email outreach" text="Create a short list of roles, draft tailored emails, and review them before sending." /><div className="aa-prerequisites"><span className="aa-prereq aa-prereq-ready">✓ Résumé parsed</span><span className="aa-prereq">○ Groq key · Profile or Connections</span><span className="aa-prereq">○ Gmail connected</span></div><section className="aa-panel"><SectionHeader eyebrow="Step 1 · Research" title="Create a research brief" text="Copy the brief into Claude, ChatGPT, or Gemini. It asks for distinct roles and contact emails that can be imported here." /><form className="aa-form" onSubmit={generatePrompt}><div className="aa-field-grid"><Field label="Target role (optional)" value={targetRole} onChange={setTargetRole} placeholder="AI Engineer or Backend Engineer" /><Field label="Location" value={location} onChange={setLocation} placeholder="New Delhi, India" /></div><label className="aa-checkbox"><input type="checkbox" checked={remoteOnly} onChange={(event) => setRemoteOnly(event.target.checked)} /><span>Remote roles only</span></label><Button type="submit" busy={busy === "prompt"} className="aa-button-primary">Generate research prompt</Button></form>{prompt?.prompt && <div className="aa-prompt-output"><div className="aa-section-header"><div><strong>Copy into Claude, ChatGPT, or Gemini</strong><small>{stringValue(prompt.summary, "100-lead research brief")}</small></div><Button onClick={() => void copyPrompt()} className="aa-button-secondary">Copy prompt</Button></div><textarea value={prompt.prompt} readOnly rows={18} aria-label="External AI research prompt" /></div>}</section><section className="aa-panel"><SectionHeader eyebrow="Step 2 · Import" title="Upload the completed workbook" text="Choose the CSV or XLSX created by Claude, ChatGPT, or Gemini." /><form className="aa-import-card" onSubmit={importWorkbook}><label className="aa-dropzone aa-dropzone-small"><input type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => setFile(event.target.files?.[0] || null)} /><span className="aa-upload-icon">↑</span><strong>{file ? file.name : "Choose completed CSV or XLSX"}</strong><small>{file ? bytesLabel(file.size) : "Upload it when your workbook is ready."}</small></label><Button type="submit" busy={busy === "import"} className="aa-button-primary">Import into Jobs</Button></form></section><section className="aa-panel"><div className="aa-step-heading"><span>3</span><div><p className="aa-eyebrow">Choose roles</p><h3>Only roles with email leads</h3><p>Select up to 30 roles. The order is kept for drafting.</p></div><span className="aa-count-badge">{selected.length} / 30</span></div><div className="aa-outreach-list">{candidates.map((job) => { const index = selected.indexOf(job.id); const app = appFor(job.id); return <label className={`aa-outreach-row ${index >= 0 ? "aa-row-active" : ""}`} key={job.id}><input type="checkbox" checked={index >= 0} onChange={() => toggle(job.id)} /><span className="aa-order">{index >= 0 ? String(index + 1).padStart(2, "0") : "·"}</span><span className="aa-list-copy"><strong>{stringValue(job.title, "Untitled role")}</strong><span>{stringValue(job.company)}{job.location ? ` · ${job.location}` : ""}</span></span>{job.fit?.score ? <span className="aa-fit-pill">{job.fit.score}% fit</span> : <Status value={app?.status || job.status} />}</label>; })}{!candidates.length && <Empty title="No roles with email leads" text="Import a workbook with an email column first. Roles without an address stay out of this list." />}</div></section><section className="aa-panel"><div className="aa-step-heading"><span>4</span><div><p className="aa-eyebrow">Draft and send</p><h3>What do you want to do?</h3><p>Pick an email above, then draft. Nothing is sent without your approval.</p></div></div><div className="aa-button-row"><Button onClick={() => void createDrafts()} busy={busy === "drafts"} disabled={!selected.length} className="aa-button-primary">Draft selected emails</Button><Button onClick={() => void findContacts()} busy={busy === "contacts"} disabled={!selected.length} className="aa-button-secondary">Find more public contacts</Button><Button onClick={() => void sendApproved()} busy={busy === "send"} disabled={!selected.length} className="aa-button-secondary">Queue approved emails</Button></div><div className="aa-contact-list">{selected.map((id) => { const job = jobs.find((item) => item.id === id); if (!job) return null; const found = contacts[id] || []; const application = appFor(id); return <div className="aa-contact-row" key={id}><div className="aa-list-copy"><strong>{stringValue(job.company)}</strong><span>{found.length ? `${found.length} email lead${found.length === 1 ? "" : "s"}` : "Email lead ready"}</span></div><div className="aa-contact-email-area">{found.length ? <select value={chosen[id] || job.contact_email || found[0]?.email || ""} onChange={(event) => setChosen((current) => ({ ...current, [id]: event.target.value }))}><option value="">Choose an email</option>{job.contact_email && !found.some((contact) => contact.email === job.contact_email) && <option value={job.contact_email}>{job.contact_email}</option>}{found.map((contact) => <option value={contact.email} key={`${contact.email}-${contact.name}`}>{contact.email}{contact.name ? ` · ${contact.name}` : ""}</option>)}</select> : <span className="aa-muted">{job.contact_email || "No email lead"}</span>}</div><div className="aa-contact-row-actions">{application ? <button type="button" className="aa-button-link" onClick={() => openDraft(application.id)}>View draft</button> : <span className="aa-muted">Not drafted</span>}</div></div>; })}{!selected.length && <Empty title="Select a role with an email" text="Only roles with an available email address can be selected." />}</div></section>{applications.some((application) => application.channel === "email" && Boolean(application.recipient || jobs.find((job) => job.id === application.job_id)?.contact_email)) && <ApplicationsView client={client} applications={applications} jobs={jobs} setApplications={setApplications} notify={notify} refresh={refresh} embedded focusApplicationId={focusApplicationId} />}</div>;
}

function ConnectionsView({ client, connections, credentials, setCredentials, notify, refresh }: ViewProps) {
  const [busy, setBusy] = useState("");
  ...
  const saveCredential = async (event: React.FormEvent) => { event.preventDefault(); setBusy("credential"); try { const result = await apiRequest<{ data?: Credential }>(client, `/provider-credentials/${encodeURIComponent(provider)}`, { method: "PUT", body: { api_key: apiKey, ...(provider === "browserbase" ? { project_id: projectId } : {}) } }); const saved = unwrapData(result); if (saved?.provider) setCredentials((current) => [...current.filter((item) => item.provider !== saved.provider), saved]); setApiKey(""); setProjectId(""); await refresh(); notify(`${humanize(provider)} credential saved to your account.`, "success"); } catch (error) { notify(errorMessage(error, "Credential validation failed."), "error"); } finally { setBusy(""); } };
